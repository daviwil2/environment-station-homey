...
module.exports = class EnvironmentStationDevice extends Homey.Device {
  ...
  _formatTimestamp(epochSeconds){
    ...
    return hours+':'+minutes; // we want just the time, not the date, due to space limitations in the Homey UI
  ...
  onInit(){
    ...
    client.on('message', function(topic, message, packet){
      ...
      if (capability && capabilities.includes(capability)){
        ...
        switch (capability){
          case 'timestamp':
            message = this._formatTimestamp.call(this, message);  // if timestamp convert epoch seconds to time hh:mm
            break;
          case 'measure_uvi':
            message = (message * 40); // if UVIndex then convert WM^2 to UV Index range [0-16]
            break;
        }; // switch

        this._log('MQTT message received: '+capability+' value '+message);
